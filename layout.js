(() => {
  if (window.SwedsnusLayout) return;

  const Core = window.SwedsnusCore;
  if (!Core) throw new Error('SwedsnusCore must load before layout.js');

  const { $, $$, keys, readStore, loggedIn, parsePrice, escapeHtml } = Core;
  ...
  const isCatalog = key => key === 'catalog' || CATALOG_LINKS.some(([id]) => id === key);
  const active = condition => condition ? ' class="active"' : '';
  const catalogDropdown = () => `<ul class="sub-dropdown">${CATALOG_LINKS.map(([, label, href]) => `<li><a href="${href}">${label}</a></li>`).join('')}</ul>`;
  ...
  function primaryNav(key) {
    return `<nav class="subheader" aria-label="Huvudnavigation"><ul class="subheader-inner">${MAIN_LINKS.map(([id, label, href]) => {
      ...
      return id === 'catalog'
        ? `<li class="has-dropdown"><a href="#"${active(isActive)}>${label} ${CHEVRON}</a>${catalogDropdown()}</li>`
        : `<li><a href="${href}"${active(isActive)}>${label}</a></li>`;
    }).join('')}</ul></nav>`;
  }
  ...
  function renderThemeSwitcher() {
    ...
    switcher.innerHTML = `<p>Tema</p>${THEME_OPTIONS.map(([id, className, title]) => `<button class="theme-dot ${className}" type="button" data-theme="${id}" title="${escapeHtml(title)}" aria-label="${escapeHtml(title)}"></button>`).join('')}`;
  ...
  function syncCounters() {
    ...
    const saved = loggedIn() ? readStore(keys.bookmarks).length : 0;
  ...
  const menuLink = (href, label, extra = '') => `<a class="hamburger-link ${extra}" href="${href}"><span>${label}</span><span class="hamburger-arrow" aria-hidden="true">›</span></a>`;
